...
func TestSelect_Evaluates(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			inputCollection: system.Collection{},
			inputArgs:       []expr.Expression{&expr.LiteralExpression{Literal: system.Boolean(true)}},
			wantCollection:  system.Collection{},
		},
		{
			name:            "projection using empty expression",
			inputCollection: slices.MustConvert[any](address),
			inputArgs:       []expr.Expression{&expr.LiteralExpression{}},
			wantCollection:  system.Collection{},
		},
		{
			...
			wantCollection:  slices.MustConvert[any]([]*dtpb.String{address[0].GetState(), address[1].GetState()}),
		},
		{
			name:            "projection flattens output collections",
			inputCollection: slices.MustConvert[any](address),
			inputArgs:       []expr.Expression{&expr.FieldExpression{FieldName: "line"}},
			wantCollection: slices.MustConvert[any]([]*dtpb.String{
				address[0].GetLine()[0],
				address[0].GetLine()[1],
				address[1].GetLine()[0],
				address[1].GetLine()[1],
		...
		},
		{
			...
			inputCollection: slices.MustConvert[any](address),
			inputArgs: []expr.Expression{
				&expr.ExpressionSequence{
					Expressions: []expr.Expression{
						&expr.FunctionExpression{Fn: impl.First},
						&expr.FieldExpression{FieldName: "use"},
					},
				},
			},
			wantCollection: system.Collection{},
